...
namespace {
    ...
    size_t find_case_insensitive(const std::string &haystack, const char *needle, size_t start_pos = 0)
    {
        if (start_pos > haystack.size()) {
            return std::string::npos;
        }
        const char *hc = haystack.c_str();
        const char *where = strcasestr(hc + start_pos, needle);
        return where == nullptr ? std::string::npos : where - hc;
    }
