...
#ifndef lda_bumping_hpp
#define lda_bumping_hpp
...
public:
  ...
  int run_lda_bumping(bool compute_elbo, MatrixXd& lambda_init)
  {
    ...
    for (int i = 0; i < max_itr_; ++i)
    {
      ...
      update_lambda(train_corpus_, num_bootstrap_, step_size_);
      ...
      my_file.open(results_dir_ + "/runtime_bumping.dat",
                   std::ios_base::app);
      if (my_file.is_open())
      {
        my_file
      ...
      my_file.open(results_dir_ + "/lambda_bumping.dat");
      if (my_file.is_open())
      {
        my_file << lambda_;
      }
