...
public:
  ...
  int run_lda_svi(bool compute_elbo, MatrixXd& lambda_init)
  {
    ...
    for (int i = 0; i < max_itr_; ++i)
    {
      ...
      train_corpus_.sample_from_docs_into_view(minibatch_size_);
      ...
      my_file.open(results_dir_ + "/runtime_svi.dat",
                   std::ios_base::app);
      if (my_file.is_open())
      {
        my_file
      ...
      my_file.open(results_dir_ + "/lambda_svi.dat");
      if (my_file.is_open())
      {
        my_file << lambda_;
      }
